//
//  MarioPlayerDownloadURLSessionManager.swift
//  MarioVideoPlayer
//
...
public protocol MarioPlayerDownloadeURLSessionManagerDelegate: NSObjectProtocol {
...
open class MarioPlayerDownloadURLSessionManager: NSObject, URLSessionDataDelegate {
    
    fileprivate let kBufferSize = 10 * 1024
    fileprivate var bufferData = NSMutableData()
    fileprivate let bufferDataQueue = DispatchQueue(label: "com.marioplayer.bufferDataQueue")
    
    open weak var delegate: MarioPlayerDownloadeURLSessionManagerDelegate?
    
    public init(delegate: MarioPlayerDownloadeURLSessionManagerDelegate?) {
