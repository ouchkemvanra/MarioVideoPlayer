...
import Foundation
import UIKit
open class MarioPlayerCacheMediaWorker: NSObject {
    open fileprivate(set) var cacheConfiguration: MarioPlayerCacheMediaConfiguration?
    ...
    public init(url: URL) {
        let path = MarioPlayerCacheManager.cacheFilePath(for: url)
        filePath = path
        ...
        if err == nil {
            ...
            if err == nil {
                ...
                do {
                    ...
                    cacheConfiguration = MarioPlayerCacheMediaConfiguration.configuration(filePath: path)
    ...
    open func cachedDataActions(forRange range:NSRange) -> Array<MarioPlayerCacheAction> {
        var actions = [MarioPlayerCacheAction]()
            ...
            for (_, value) in cachedSegments.enumerated() {
                ...
                if intersctionRange.length > 0 {
                    ...
                    for i in 0...package {
                        ...
                        let action = MarioPlayerCacheAction(type: .local, range: ra)
        ...
        if actions.count == 0 {
            let action = MarioPlayerCacheAction(type: .remote, range: range)
            actions.append(action)
        } else {
            var localRemoteActions = [MarioPlayerCacheAction]()
            for (index, value) in actions.enumerated() {
                ...
                if index == 0 {
                    if range.location < actionRange.location {
                        ...
                        let action = MarioPlayerCacheAction(type: .remote, range: ra)
    ...
    open func set(cacheMedia: MarioPlayerCacheMedia) -> Bool {
